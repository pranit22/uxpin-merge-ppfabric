...
export class MarqueeSelectionBasicExample extends React.Component {
    constructor(props) {
        super(props);
        this._onChange = (ev, isMarqueeEnabled) => {
    ...
    render() {
        return (<MarqueeSelection selection={this._selection} isEnabled={this.state.isMarqueeEnabled}>
        ...
        <ul className={styles.photoList}>
          {PHOTOS.map((photo, index) => (<div key={index} className={css(styles.photoCell, this._selection.isIndexSelected(index) && 'is-selected')} data-is-focusable={true} data-selection-index={index} onClick={this._log('clicked')} style={{ width: photo.width, height: photo.height }}>
              {index}
            </div>))}
        </ul>
      </MarqueeSelection>);
